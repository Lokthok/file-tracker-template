...
def get_file_info(filepath):
    """Returns file size and modification timestamp."""
    stat = filepath.stat()
    return {
        "size": stat.st_size,
...
def main():
    ...
    for filepath, fileinfo in current_files.items():
        if filepath not in old_inventory:
            ...
            if (fileinfo["size"] != old_info.get("size") or 
                fileinfo["modified"] != old_info.get("modified")):
                # File changed
                new_inventory[filepath] = {
                ...
                # Unchanged
                new_inventory[filepath] = old_info
